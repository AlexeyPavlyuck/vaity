...
@section('content')
    ...
    <form action="{{route('post.store')}}" class="form-article" method="post" enctype="multipart/form-data">
        @csrf
        <textarea onkeyup="this.style.height = 'auto'; this.style.height = this.scrollHeight + 'px'"  placeholder=" Заголовок статьи..." id="title" type="text" name="title" required></textarea>
        <textarea onkeyup="this.style.height = 'auto'; this.style.height = this.scrollHeight + 'px'" placeholder=" Описание статьи... Максимум 100 символов" id="description" type="text" name="description" required></textarea>
